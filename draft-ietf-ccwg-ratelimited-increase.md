...
- cwnd-limited: A flow that has sent the maximum number of segments permitted by the cwnd, where the application utilises the allowed sending rate (based on the definition for TCP in Section 3 of {{?RFC7661}}).
- rate-limited: a flow that is either idle or sends at a rate less than the maximum permitted by the cwnd. (Note: Section 3 of {{?RFC7661}} provides a more specific definition applied to the reduction of the cwnd when a sender remains rate-limited for a period of time).
...
When a sender using a congestion control algorithm increases cwnd, the following "Rate-Limited Increase" rules apply:
