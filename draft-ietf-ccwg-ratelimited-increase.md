...
According to {{!RFC4341}}, a DCCP CCID specifying TCP-like behaviour is allowed to grow an unlimited cwnd during an uncongested period when it sends at a rate unconstrained by the current cwnd.
...
{{!RFC9002}} is updated by this document by replacing the final line in the cited text with the text in ({{rules}}) to specify the management of the
cwnd when the sender is rate-limited.
...
{{!RFC9260}} is updated by this document by replacing the  the final line of quoted text with the text in ({{rules}}) to specify the management of the cwnd when the sender is rate-limited.
